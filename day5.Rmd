...
pages_split <- str_split(pages_test,",")
rules_split <- str_split(rules_test,"\\|")

x <- str_match_all(pages_split[[1]],rules_split[[1]][1])
y <- str_match_all(pages_split[[1]],rules_split[[1]][2])

logicals <- vector(mode = "list", length = length(pages_split))
tests <- vector(mode = "list", length = length(rules_split))
